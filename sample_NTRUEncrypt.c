...
// static uint8_t const test_text[] = {
//     0xf3, 0xe9, 0x87, 0xbb, 0x18, 0x08, 0x3c, 0xaa,
//     0x7b, 0x12, 0x49, 0x88, 0xaf, 0xb3, 0x22, 0xd8
// };

// static uint8_t test_text[1000] ;//= "ABCDERFADFGDFGDBVDBDGERWW";//{
//     0xf3, 0xe9, 0x87, 0xbb, 0x18, 0x08, 0x3c, 0xaa, 0xfa,0xfb,0xfc,0xaa,0x01,0x22,
//     0x7b, 0x12, 0x49, 0x88, 0xaf, 0xb3, 0x22, 0xd8
// };
...
main(int argc, char *argv[])
{
    ...
    uint8_t plaintext[100];            /* size of AES-128 key */
    ...
    uint8_t test_text[60];

    // for(int i=0; i!=60; i++)
    // {
    //     test_text[i] = argv[1][i];

    // }

    printf("len %d\n", strlen(argv[1]));

    strcpy(test_text, argv[1]);




    // uint8_t  test_text[] = argv[1];
    // text[15]='\0';
    // test_text = malloc((strlen(text)+1) * sizeof(uint8_t));
    // strcpy(test_text, text);
    // test_text[strlen(text)] = '\0';
    // test_text[strlen(text)+1] = '\0';
    // test_text[strlen(text)-1] = '\0';
    ...
    printf("public_key && public_key_len %s; %d\n", public_key, public_key_len);
    ...
    rc = ntru_crypto_ntru_encrypt(drbg, public_key_len, public_key,
                            0, NULL,
                            &ciphertext_len, NULL);
    if(rc != NTRU_RESULT(NTRU_OK)) {printf("\n NTRUENCRYPT\n");}

    /* Allocate memory for ciphertext */
    // ...
    /* Perform the encryption */
    rc = ntru_crypto_ntru_encrypt(drbg, public_key_len, public_key,
                            sizeof(test_text), test_text,
...
return 1;
     ...
     */
    // rc = ntru_crypto_ntru_encrypt(drbg, public_key_len, public_key,
                                  // sizeof(test_text), test_text, &ciphertext_len,
                                  // NULL);
    // if (rc != NTRU_OK)
        /* An error occurred requesting the buffer size needed. */
        // goto error;
    ...
    // {
      ...
      // fwrite(test_text,sizeof(test_text),1,Handle);
      // fclose(Handle);
    // }
    ...
    // printf("Plaintext:\n");
    // DumpHex(test_text,sizeof(test_text));
    ...
    // if(plaintext_len!=sizeof(test_text))
    // {
    //     fprintf(stderr,"Error: Decrypted length does not match original plaintext length\n");
    //     return 1;
    // }
    // if(memcmp(plaintext,test_text,sizeof(test_text)))
    // {
